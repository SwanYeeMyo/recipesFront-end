            ...
            <tbody>
              {recipes.map((recipe, index) => (
                <tr key={recipe.id}>
                  ...
                  <td className="px-6 py-4">{recipe.user?.name}</td>
                  <td className="px-6 py-4 flex">
                    <Link
                      to={`/dashboard/recipes/${recipe.id}/update`}
                      className="font-medium text-blue-600 dark:text-blue-500 hover:underline me-4"
                    >
                      Edit
                    </Link>
                    <button
                      onClick={() => { setOpenModal(true); setClickID(recipe.id) }}
